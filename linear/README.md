...
本稿は、大学の一年生ないし二年生で、線形代数を学んでいる or 学んだけど、何に使うかわからないので学ぶモチベーションがぼんやりしている、という学生さんを対象に書いています。以下、かなりいい加減な書き方をするので、「線形代数が重要なのは当然だろ」と思っている人とか、特に数学ガチ勢な皆さんはブラウザの「戻る」ボタンを押してください。
...
1.0が出てきました。すなわち、この行列の最大固有値は1です。`linalg.eigh(A, eigvals=(N-2,N-2))`とすると、二番目に大きな固有値を得ることができますが、その値は$0.99615706$です。つまり、最大固有値が$\lambda_1 = 1$で、それ以外はすべて真に1より小さい、つまり$\lambda_i < 1 (i\neq 1)$が成り立ちます。従って、何度も$A$をかけると、最大固有値に対応するベクトル以外の成分は消えます。
